...
- **全景 3D 交互与漫游**：
  - **键盘方向键 / WASD 漫游**：支持键盘 `↑ / ↓ / ← / →` 或 `W / A / S / D` 平滑前后左右移动镜头，按住 `Shift` 加速漫游。
  - **Q / E 原地左旋与右旋**：支持 `Q` 原地向左旋转视线，`E` 原地向右旋转视线，配合前进键即可自然弧线行走。
  - **垂直原地上升与下降**：支持快捷键 `Space` 原地上升，`C / Z / Shift+Space` 原地下降，或直接点击左下角界面的 `🔼 原地上升` 与 `🔽 原地下降` 按钮。
  - **左下角操作浮窗与侧边栏缩进**：
    - 支持一键 `◀ 收起` 将说明浮窗缩进至左侧边栏，折叠为小巧标签，释放全屏纯净视野，点击随时呼出。
    - 内置移动尺度/速度滑轮（滑块拖拽 + 鼠标滚轮微调 + 4 档预设倍率），自适应大场景与微观特写。
  - **鼠标交互**：左键以光标位置为中心旋转、双击表面快速聚焦、右键平移、滚轮无级缩放。
